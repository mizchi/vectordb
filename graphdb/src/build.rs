...
//! `vectordb` index) to get semantic edges, optionally keep only reciprocal
//! (mutual-kNN) edges, then merge explicit links. This is where vectordb does
//! the heavy lifting — graphdb only turns neighbor lists into a weighted graph.
...
impl GraphBuilder {
    ...
    pub fn build(&self, items: &[(u64, Vec<f32>)], links: &[(u64, u64)]) -> GraphStore {
        ...
        // 1) Semantic kNN via a vectordb HNSW index.
