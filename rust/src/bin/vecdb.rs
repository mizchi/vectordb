...
//! `id,v0,v1,...,vD-1`. The command logic lives in [`vectordb::cli`] over an
//! `Env` abstraction; this binary just wires it to the real filesystem
//! ([`vectordb::cli::StdEnv`]). The one exception is the low-memory DiskANN
...
//!   vecdb search       <index.vecdb> <query.csv> [-k N] [--oversample M] [--nprobe N] [--ef N]
//!   vecdb info         <index.vecdb>

use std::fs::File;
use std::io::{BufRead, BufReader};
use std::process::ExitCode;
use vectordb::cli::{self, flag_value, has_flag, parse_flag, parse_metric, StdEnv};
