...
fn main() {
    ...
    let items: Vec<(u64, Vec<f32>)> = base.iter().cloned().enumerate().map(|(i, v)| (i as u64, v)).collect();
    ...
    {
        let f = &flat;
        eval("flat int8+rerank (o=8)", Box::new(move |q| f.search(q, k, 8)));
    }
    ...
    {
        let b = &bin;
        eval("binary 1-bit+rerank (o=32)", Box::new(move |q| b.search(q, k, 32)));
    }
    ...
    {
        let r = &rq;
        eval("RaBitQ flat+rerank (o=32)", Box::new(move |q| r.search(q, k, 32)));
    }
    ...
    for &np in &[4usize, 8, 16] {
        let i = &ivf;
        eval(&format!("IVF nprobe={np} (o=8)"), Box::new(move |q| i.search(q, k, np, 8)));
    }
    ...
    for &np in &[8usize, 16] {
        let i = &ivfrq;
        eval(&format!("IVF+RaBitQ nprobe={np} (o=32)"), Box::new(move |q| i.search(q, k, np, 32)));
    }
    ...
    for &ef in &[16usize, 32, 64, 128] {
        let h = &hnsw;
        eval(&format!("HNSW efSearch={ef}"), Box::new(move |q| h.search(q, k, ef)));
    }
}
